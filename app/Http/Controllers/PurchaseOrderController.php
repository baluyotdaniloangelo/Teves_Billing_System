...
class PurchaseOrderController extends Controller
{
	...
	public function purchaseorder(){
		...
		return view("pages.purchase_order.index", compact('data','title','product_data','purchase_data_suggestion','purchase_payment_suggestion','supplier_data','teves_branch','product_category_data'));
...
';
	...
	public function purchase_order_form($PurchaseOrderID){
		...
		//return view("pages.purchase_order_form", compact('data','title','supplier_data','teves_branch', 'purchase_data_suggestion','purchase_payment_suggestion', 'PurchaseOrderID', 'product_category_data'));
		return view("pages.purchase_order_form.index",
			compact('data', 
			'title',
			'supplier_data',
			'teves_branch', 
			'purchase_data_suggestion',
			'purchase_payment_suggestion', 
			'PurchaseOrderID', 
	...
	public function purchase_order_info(Request $request){
				...
				->leftJoin('teves_branch_table', 'teves_branch_table.branch_id', '=', 'teves_purchase_order_table.company_header')
              	->get([
						'teves_supplier_table.supplier_name',
						'teves_purchase_order_table.category_idx',
	...
	public function create_purchase_order_post(Request $request){
		...
		$request->validate([
			...
			'category_idx'		=> 'required'
        ], 
        [
			'supplier_idx.required' 	=> "Supplier's Name is Required",
			'category_idx.required' 	=> "Purchase Order Type Required"
        ]
		);
			...
			$Purchaseorder->category_idx							= $request->category_idx;
	...
	public function update_purchase_order_post(Request $request){
        ...
        [
			...
			'purchase_order_type.required' 	=> "Purchase Order Type is Required"
        ]
		);
		//
			...
			$Purchaseorder->purchase_order_control_number 			= $control_number;
			$Purchaseorder->purchase_order_date 					= $request->purchase_order_date;
			$Purchaseorder->category_idx 							= $request->purchase_order_type;
			$Purchaseorder->purchase_order_supplier_idx				= $request->supplier_idx;
	...
	public function generate_purchase_order_summary_report_consolidated_pdf(Request $request){
		...
		$purchase_order_data = PurchaseOrderModel::leftJoin('teves_supplier_table', 'teves_supplier_table.supplier_id', '=', 'teves_purchase_order_table.purchase_order_supplier_idx')
		->leftJoin('teves_branch_table', 'teves_branch_table.branch_id', '=', 'teves_purchase_order_table.company_header')
		->where('teves_purchase_order_table.purchase_order_less_percentage', '<>', 0)
		->whereBetween('teves_purchase_order_table.purchase_order_date', [$start_date, $end_date])
		...
		$purchase_order_without_withholding_tax_data = PurchaseOrderModel::leftJoin('teves_supplier_table', 'teves_supplier_table.supplier_id', '=', 'teves_purchase_order_table.purchase_order_supplier_idx')
		->leftJoin('teves_branch_table', 'teves_branch_table.branch_id', '=', 'teves_purchase_order_table.company_header')
		->where('teves_purchase_order_table.purchase_order_less_percentage', '=', 0)
		->whereBetween('teves_purchase_order_table.purchase_order_date', [$start_date, $end_date])
		->when($company_header != 'All', function ($q) use ($company_header) {
		...
		$title = 'Purchase Order - Summary';
        $pdf = PDF::loadView('printables.report_purchase_order_summary_consolidated_pdf', compact('title', 'purchase_order_data', 'user_data','receivable_header','start_date','end_date','supplier_data', 'purchase_order_without_withholding_tax_data'));
		/*Download Directly*/
        //return $pdf->download($client_data['client_name'].".pdf");
		/*Stream for Saving/Printing*/
		$pdf->setPaper('legal', 'landscape');/*Set to Landscape*/
	...
	public function generate_purchase_order_summary_report_per_client_consolidated_pdf(Request $request){
		...
		$title = 'Purchase Order - Summary';
        $pdf = PDF::loadView('printables.report_purchase_order_summary_per_client_consolidated_pdf', compact('title', 'purchase_order_data', 'user_data','receivable_header','start_date','end_date','supplier_data', 'purchase_order_without_withholding_tax_data'));
		/*Download Directly*/
        //return $pdf->download($client_data['client_name'].".pdf");
		/*Stream for Saving/Printing*/
		$pdf->setPaper('legal', 'landscape');/*Set to Landscape*/
