   ...
   <script type="text/javascript">
	...
	$(function () {
		...
		/*Get List Of Undelivered Sales Order*/
		var SalesOrderListTable_delivered = $('#getSalesOrderList_delivered').DataTable({
			"language": {
						"lengthMenu":'<select class="dt-input">'+
			             '<option value="10">10</option>'+
			             '<option value="20">20</option>'+
			...
			scrollCollapse: true,
			scrollY: '500px',
			ajax: "{{ route('getSalesOrderList_delivered') }}",
			columns: [
				{data: 'DT_RowIndex', name: 'DT_RowIndex' , orderable: false, searchable: false},
				{data: 'sales_order_date'},
				{data: 'sales_order_control_number'},
			...
			columnDefs: [
				...
				autoAdjustColumns(SalesOrderListTable_delivered);
