...
function initializePurchaseOrderTable() {

    PurchaseOrderTable =
        $('#getPurchaseOrderList').DataTable({
...
function showDeletePurchaseOrderModal(event) {
        ...
        success: function (response) {
            ...
            if (!response || !response[0]) {
                ...
                showDeleteModalError(
                    'Purchase Order information not found.'
                );
            ...
            $('#confirm_delete_purchase_order_date')
                .text(data.purchase_order_date || '-');

            $('#confirm_delete_purchase_control_number')
                .text(data.purchase_order_control_number || '-');

            $('#confirm_delete_suppliers_name')
                .text(data.purchase_supplier_name || '-');

            $('#confirm_delete_amount')
                .text(
                    formatCurrency(
                        data.purchase_order_total_payable || 0
                    )
                );
...
function deletePurchaseOrderConfirmed(event) {
    ...
    $.ajax({
        ...
        url:
            '/delete_purchase_order_confirmed',
...
function reloadPurchaseOrderTable() {
...
function getSelectedSupplierId() {
    ...
    return $("#supplier_name_create option[value='" +
            $('#supplier_idx_create').val() +
            "']")
            .attr('data-id');
}
...
function redirectToPurchaseOrderForm(id) {

    const url =
        "{{URL::to('purchase_order_form')}}";
...
function resetDeleteModal() {
    ...
    $('#confirm_delete_purchase_order_date').text('-');

    $('#confirm_delete_purchase_control_number').text('-');

    $('#confirm_delete_suppliers_name').text('-');

    $('#confirm_delete_amount').text('-');
}
...
function ViewGalery(purchase_order_id)
{
    ...
    $.ajax({
        ...
        success: function(response)
        {
            ...
            $.each(response, function(index, item)
            {
                ...
                let activeCurrent = (index === 0) ? 'true' : 'false';

                /* IMAGE */
                let image_src = '';

                if(item.image_reference == null || item.image_reference == '')
                {
                ...
                {	
					image_src = "data:image/jpg;image/png;base64,"+item.image_reference;
                    //image_src = item.image_reference;
                }
                            ...
                            <div class="position-absolute bottom-0 start-0 end-0 p-4"
                                ...
                                <div class="row text-white">
                                    ...
                                    <div class="col-md-3 mb-2">
                                        ...
                                        <div class="fw-semibold">
                                            ${item.purchase_order_date_of_payment ?? '-'}
                                        </div>
                                    ...
                                    <div class="col-md-3 mb-2">
                                        ...
                                        <div class="fw-semibold">
                                            ${item.purchase_order_reference_no ?? '-'}
                                        </div>
                                    ...
                                    <div class="col-md-3 mb-2 text-md-end">
                                        ...
                                        <div class="fw-bold fs-5 text-warning">
                                            ₱ ${parseFloat(item.purchase_order_payment_amount || 0).toLocaleString('en-PH', {
                                                minimumFractionDigits: 2
                                            })}
                                        </div>
	...
	function check_withholding_tax(){
			...
			if(purchase_order_invoice==1){
				...
				document.getElementById("purchase_order_less_percentage_create").value = 0;
			}
	...
	function SupplierInfo(){
			...
			let supplierID = $("#supplier_name_create option[value='" + $('#supplier_idx_create').val() + "']").attr('data-id');
			  ...
			  $.ajax({
				...
				success:function(response){
				  ...
				  if(response) {
					...
					document.getElementById("purchase_order_less_percentage_create").value = response.default_withholding_tax_percentage;
					document.getElementById("purchase_order_net_percentage_create").value = response.default_net_percentage;				
